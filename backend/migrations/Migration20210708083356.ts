...
export class Migration20210708083356 extends Migration {
  ...
  async up(): Promise<void> {
    ...
    this.addSql('create table `article` (`id` int unsigned not null auto_increment primary key, `create_time` datetime not null, `last_update_time` datetime not null, `owner_set_publicity` tinyint(1) not null, `admin_set_publicity` tinyint(1) not null, `owner_id` int(11) unsigned not null) default character set utf8mb4 engine = InnoDB;');
    this.addSql('alter table `article` add index `article_owner_id_index`(`owner_id`);');

    this.addSql('create table `article_revision` (`id` int unsigned not null auto_increment primary key, `revision_number` int(11) not null, `time` datetime not null, `title` varchar(255) not null, `authors` json not null, `keywords` text not null, `abstract` varchar(500) not null, `category` varchar(255) not null, `pdf_id` int(11) unsigned not null, `article_id` int(11) unsigned not null, `latest_revision_of_id` int(11) unsigned null, `code_link` varchar(255) null) default character set utf8mb4 engine = InnoDB;');
